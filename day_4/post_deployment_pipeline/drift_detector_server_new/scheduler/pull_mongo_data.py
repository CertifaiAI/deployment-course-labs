...
def mongo_auto_query():
    ...
    while(True):
        ...
        all_documents = mongodb.image_features_sequence_new.find({}, {'id':1, '_id':0})
        ...
        # if there are image_features_new within the index_range, query image_features_new and send request to drift_detector API endpoint
        if mongodb.image_features_new.count_documents(myquery) != 0:
            logging.info("document exists")
        
            mydoc = mongodb.image_features_new.find(myquery)
            
            # unpickle into tensor
            tensor_list = [pickle.loads(x['image_features_new']) for x in mydoc]
